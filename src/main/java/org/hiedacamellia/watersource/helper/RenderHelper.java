...
public class RenderHelper {
    ...
    public static void renderFluidAmount(SimpleAnimationBlockEntity blockEntity, Player player, String text, PoseStack matrixStackIn, float partialTicks, float animDuration){
        ...
        Quaternionf quaternion = blockEntity.getQuaternion();
        ...
        switch (direction) {
            case SOUTH:
                ...
                break;
            case NORTH:
                matrixStackIn.mulPose(new Quaternionf(0, 180, 0, 0));
                matrixStackIn.translate(-0.5 - a, 0.25, 0.1);
                break;
            case EAST:
                matrixStackIn.mulPose(new Quaternionf(0, 90, 0, 0));
                matrixStackIn.translate(-0.5 - a, 0.25, 1.05);
                break;
            case WEST:
                matrixStackIn.mulPose(new Quaternionf(0, 270, 0, 0));
        ...
        Font font = mc.font;
        font.drawInBatch(text, 0, 0, 0xFFFFFF, false, matrixStackIn.last().pose(), mc.renderBuffers().bufferSource(), Font.DisplayMode.NORMAL, 0, 15728880);
